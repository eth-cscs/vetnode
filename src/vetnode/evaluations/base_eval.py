...
class BaseEval(EvalConfiguration):
    ...
    async def eval(self)->EvalResult:
        result:EvalResult = EvalResult(**{"rank":self.context.rank, "eval_id":self.context.eval_id, "eval_name":self.name, "eval_type": self.type, "elapsedtime":0, "status":EvalResultStatus.UNKNOWN, "metrics":None, "hostname":self.context.hostname})
